...
if (strlen($_SESSION['emplogin']) == 0) {
    ...
    if (isset($_POST['change'])) {
        ...
        if ($_POST['confirmpassword'] != $_POST['newpassword']) {
            echo "<script>
            swal('รหัสผ่านของท่านไม่ตรงกัน', 'โปรดตรวจสอบข้อมูลอีกครั้ง', 'warning').then(
                function() {
                  window.location.href = 'emp-changepassword.php';
                }
              );
            </script>";
        } else {
            ...
            if ($query->rowCount() > 0) {
                ...
                $chngpwd1->execute();
                echo "<script>
                swal('สำเร็จ', 'ท่านได้ทำการเปลี่ยนรหัสผ่านเรียบร้อยแล้ว', 'success').then(
                    function() {
                      window.location.href = 'emp-changepassword.php';
                    }
                  );
                ...
                echo "<script>
                swal('รหัสผ่านเดิมของท่านไม่ถูกต้อง', 'โปรดตรวจสอบข้อมูลของท่านอีกครั้ง', 'warning').then(
                    function() {
                      window.location.href = 'emp-changepassword.php';
                    }
                  );
